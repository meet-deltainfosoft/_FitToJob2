...
public partial class API_GetCheckedQnListWithAns : System.Web.UI.Page
{
    string SubId;
    string TestId;
    ...
    public string selectdata()
    {
        ...
        try
        {
            ...
            da = _aPI_BLL.returnDataTable(" select q.SubId,q.QueId,q.Que as Question ,q.A1,q.A2,q.A3,q.A4  ,s.Name as Subject, newid() as 'OrderBy' " +
                                          ...
                                          " , StartDt = (select top 1 StartDt from ExamStartStopTimes where RegistrationId = el.RegistrationId " +
                                          " and ExamScheduleId = e.ExamScheduleId and EndDt is null order by InsertedOn desc) " +
                                          " ,q.Srno as QueNo,e.PerQueMins as QnTime,e.TotalMins, ts.TestId,ee.Ans,ee.AnsImage1,ee.AnsImage2, ee.AnsImage3, ee.AnsImage4 ,q.ans  as TrueAns " +
                                          " , ex.ImageNo, ex.TotalObtMark, ex.Remarks " +
                                          " , replace(ex.ImagePath, '" + ConfigurationSettings.AppSettings["FolderPath"] + "', '" + ConfigurationSettings.AppSettings["FolderPathShow"] + "') as 'CheckedImagePath' " +
                                          " into #tmp From ExamSchedules e " +
                                          " inner join ExamScheduleLns el on el.ExamScheduleId = e.ExamScheduleId " +
                                          " inner join TextLists t on t.TextListId = e.StandardTextListId " +
                                          " inner join Subs s on s.SubId = e.SubId " +
                                          " inner join Tests ts on ts.TestId = e.TestId " +
                                          " inner join Ques q on s.SubId = q.SubId and ts.TestId = q.TestId " +
                                          " left join Exams ee on ee.ExamScheduleId = e.ExamScheduleId and ee.SubId = s.SubId and ee.QueId = q.QueId and el.RegistrationId = ee.RegistrationId " +
                                          " left join ExamEvaluations ex on ex.ExamId = ee.ExamId " +
                ...
                //" and q.QueId not in (select QueId from Exams where RegistrationId = '" + RegistrationId.ToString() + "' and ExamScheduleId = '" + ExamScheduleId.ToString() + "') " +
                                          ...
                                          " , QueType, QueDataType, RightMarks, WrongMarks, NonMarks, NoOfFile, PerQuestionTime, InsertedOn, StartDt, QueNo, QnTime, TotalMins, TestId,Ans,AnsImage1,AnsImage2,AnsImage3,AnsImage4,TrueAns " +
                                          " , ImageNo, TotalObtMark, Remarks, CheckedImagePath " +
                                          " from #tmp  " +
