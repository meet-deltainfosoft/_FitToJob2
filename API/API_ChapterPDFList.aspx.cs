...
public partial class API_ChapterPDFList : System.Web.UI.Page
{
    string ChapterId;
    ...
    public string selectdata()
    {
        ...
        try
        {
            da = _aPI_BLL.returnDataTable(" select ChapterPDFId, ChapterId, SrNo, FileName,  Remarks, InsertedOn, " +
                                          " LastUpdatedOn, InsertedByUserId, LastUpdatedByUserId, StandardTextListId, SubId " +
                                          " , replace(FileLink, '" + ConfigurationSettings.AppSettings["FolderPath"] + "', '" + ConfigurationSettings.AppSettings["FolderPathShow"] + "') as 'FileLink' " +
                                          " from ChapterPDFs where ChapterId = '" + ChapterId.ToString() + "'  order by SrNo ");
