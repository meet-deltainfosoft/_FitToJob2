...
public partial class API_ValidUser : System.Web.UI.Page
{
    //send sms
    string MobileNo;
    string Key;
        ...
        try
        {
            ...
            if (da.Rows.Count > 0)
            {
                ...
                foreach (DataRow dtr in da.Rows)
                {
                    ...
                    _aPI_BLL.InsertUpdateNonQuery(" Update " + dtr["DBName"] + "..TextLists set OTP = '" + OTP.ToString() + "' " +
                                                  " where [Address] = '" + da.Rows[0]["MobileNo"].ToString() + "' ");
                }
    ...
    protected void SendSMS(string MobileNo, string Key, string OTP, string DBName)
    {
        try
        {
            ...
            //Message = "<#> " + OTP + " is OTP for Digital iClass App Login. " + Key + "";
            Message = "" + OTP + " is OTP for Digital iClass App Login. " + Key + " - Vidyamandir Palanpur";

            Message = replaceSplChar(Message);

            string strUrl = _aPI_BLL.SMSString(DBName);

            strUrl = "http://ui.netsms.co.in/API/SendSMS.aspx?APIkey=b43pnRn1dlh6PXARH74HiNlsan&SenderID=TIMVVM&SMSType=2&Mobile=[MOBILENO]&MsgText=[MESSAGE]&EntityID=1101430440000012129&TemplateID=1107161883038803324";
