...
/// <summary>
/// Summary description for GetPunchNoDAL
/// </summary>
public class GetPunchNoDAL
{
	private GeneralDAL _generalDAL;

    public GetPunchNoDAL()
    {
    ...
    public DataTable CandidateFinalNot(DateTime? FromDt, DateTime? ToDt, string Name, string UserId)
    {
        ...
        sqlCmd.CommandText = " Select DISTINCT a.HODInterviewId,a.Name AS Name,a.Status,a.Remarks,a.RegistrationId,R.LastName ,i.ThirdCompanyName as LastCompany , " +
                             ...
                             " left join ExamScheduleLns es on es.RegistrationId = R.RegistrationId  " +
                             " left join ExamSchedules ess on ess.ExamScheduleId = es.ExamScheduleId  " +
    ...
    public void Update(ArrayList alAccount)
    {
        ...
        try
        {
            foreach (string Account in alAccount)
            {
                ...
                remarks = approvedAccount[1];


                sqlcmd.CommandText = " UPDATE HODInterViews SET " +
                                     " PunchNoOn=GetDate() " +
                                     ",PunchNoByUserId='" + MySession.UserUnique + "'" +
                                     ",PunchNo = " + ((remarks == null || remarks == "") ? "NULL" : "'" + remarks.Replace("'", "''") + "'") + "" +
